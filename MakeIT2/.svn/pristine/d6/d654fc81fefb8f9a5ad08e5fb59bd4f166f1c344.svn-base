...
public class SignUpController implements Initializable {
	...
	@FXML
	private JFXPasswordField tfMemPw;

	@FXML
	private JFXPasswordField tfConfMemPw;
	...
	public void initialize(URL location, ResourceBundle resources) {
		 ...
		 * 비번 맞는지 검사
		 */
		tfConfMemPw.setOnKeyReleased(new EventHandler<Event>() {
			public void handle(Event event) {
				String pw1 = tfMemPw.getText().trim();
				String pw2 = tfConfMemPw.getText().trim();
				if(pw1.length() > 0 && pw2.length() > 0){
					if(pw1.equals(pw2)) {
						tfConfMemPw.setStyle("-fx-background-color:#00FF00");
						tfMemPw.setStyle("-fx-background-color:#00FF00");
					}else {
						tfConfMemPw.setStyle("-fx-background-color:#FF4040");
						tfMemPw.setStyle("-fx-background-color:#FF4040");
					}
				}else {
					tfConfMemPw.setStyle("-fx-background-color:#FFFFFF");
					tfMemPw.setStyle("-fx-background-color:#FFFFFF");
				}
			}
		});
