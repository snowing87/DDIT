...
public class LogInController implements Initializable{
    ...
    void onLoginClick(ActionEvent event) {
    	...
    	if(blList.size() != 0) {
    		...
    		error.setTitle("니방내방");
    		error.setHeaderText("다음과 같은 사유로 니방내방을 이용하실 수 없습니다.");
    		error.setContentText("사유 : " + blList.get(0).getBlacklist_comment());
    		error.showAndWait();
    		// 로그인을 종료한다.
    		((Node)(event.getSource())).getScene().getWindow().hide();
    		return;
    	...
    	int cnt = service.checkDeletedMember(id);
    	if(cnt>0) {
    		Alert error = new Alert(AlertType.ERROR);
    		error.setTitle("니방내방");
    		error.setHeaderText("다음과 같은 사유로 니방내방을 이용하실 수 없습니다.");
    		error.setContentText("사유 : 탈퇴한 계정입니다. 다른 ID로 다시 가입해주세요.");
    		error.showAndWait();
